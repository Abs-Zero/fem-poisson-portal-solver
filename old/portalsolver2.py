...
class PortalPoissonSolver:
    ...
    def generate_domain_mesh(self, switch_portal=True):
        ...
        if switch_portal == True:
            for i, (p1, p2) in enumerate(self.portal_points):
                ...
                for (j, k, st, ed) in switch_rule:
                    ...
                    self.switched_portal_element_points[i].setdefault(j, set()).add((k, st, ed))
    ...
    def plot_mesh(
        ...
        plot_unswitched_mesh=False,
        fig_name="meshfig.svg",
    ):
        print("绘制网格图像...")
        elements = self.elements_unswitched if plot_unswitched_mesh else self.elements
        D = self.domain_side_length
    ...
    def solve_poisson(self, func_domain, func_boundary):
        ...
        # U = self.linear_solver(K, F)

        K_csr = K.tocsr()
        U, _ = self.linear_solver(K_csr, F, rtol=self.cg_tol, maxiter=self.cg_maxiter)
...
if __name__ == "__main__":
    ...
    portals = [
        [(1, 0, 1.5, np.pi / 2), (-1, 0, 1.5, np.pi)],
        [(0, 1, 1.5, np.pi / 2), (0, -1, 1.5, -np.pi / 2)],
    ]

    # portals = []

    solver = PortalPoissonSolver(portals, D, MS_REGULAR, MS_MIN)
    solver.generate_domain_mesh(switch_portal=True)
    # solver.plot_mesh()  # 网格图像
    ...
    solver.solve_poisson(func_domain_source, func_boundary)
